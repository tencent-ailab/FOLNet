...
# Input arguments:
# - tokens_a: the first sequence
# - tokens_b: the second sequence
# - max_num_tokens: the maximum number of tokens for the total sequence length
...
# the hdf5 dataset for pretraining
class Hdf5Dataset(Dataset):
    ...
    def __getitem__(self, index):
        # return the data sample (in dictionary) for a given index
        [
...
# The Dynamic Dataset, which will dynamically mask the input sequence for MLM
# Also, we implement SOP here instead of NSP for more efficient preprocessing.
# It is based on IterableDataset, which is more efficient for large data files.
class DynDataset(IterableDataset):
    ...
    def __iter__(self):
        # get the worker information for the current worker in DDP
        ...
        with open(self.corpus_path, 'rb') as data_file:
            ...
            for _ in range(worker_samples):
                ...
                # create the next-sentence prediction (NSP) data
                # (in fact, we implement SOP data here fore efficiency)
                ...
                # pad the data to maximum number of predictions
                pad_len = self.max_predictions_per_seq - len(positions)
                positions.extend([self.pad_token]*pad_len)
                labels.extend([-1]*pad_len)
                nsp = [nsp]
                # add sequence constrastive loss (SCL) if required
                # the key idea is to create another copy of SOP and MLM data as
                # and then batch them together with the original copy as the
                # positive paris; the negative are done within batch
    ...
    def _set_file_range(self, worker_info):
        # for the worker, set the starting point and total number of samples from the file
    ...
    def _parse_bytes(self, sample):
        # parse the bytes into actual data samples
        output = {
            k: np.frombuffer(
    ...
    def _create_masked_lm_predictions(
        ...
        # get all the tokens other than the special tokens
        cand_indexes = []
    ...
    def _create_nsp_predictions(
        ...
        # note that here it is actually sentence order prediction (SOP).
        # this can greatly save the preprocessing complexity.
        # The idea is we randomly find a position and the swap the sequence
