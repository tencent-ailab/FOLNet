...
from folnet2 import FOLNetForSeqClassification as FOLNetForSeqCls_PreLN_Full
from folnet2s import FOLNetForSeqClassification as FOLNetForSeqCls_PreLN_Sparse
from tokenization import BertTokenizer
from optimization import BertAdam, warmup_linear
from schedulers import LinearWarmUpScheduler
from sklearn.metrics import matthews_corrcoef, f1_score
from scipy.stats import pearsonr, spearmanr
from processors.clutrr import PROCESSORS, convert_examples_to_features


MODEL_DICT = {
    "PreLN": FOLNetForSeqCls_PreLN_Full,
    "PreLN.Full": FOLNetForSeqCls_PreLN_Full,
    "PreLN.Sparse": FOLNetForSeqCls_PreLN_Sparse,
}
...
def evaluate_model(net, args, tokenizer, processor):
    all_results = []
    for k in range(2, 11):
        result = _evaluate_model(net, args, tokenizer, processor, "1."+str(k))
        all_results.append(result["acc"])
    metrics = {"detail": {"acc": all_results}}
    # metrics["main"] = all_results[0]
    metrics["main"] = sum(all_results) / len(all_results)
    return metrics
                ...
                if (step%log_freq==0 or tr_step>=args.max_steps) and rank==0:
                    ...
                    if results["metrics"]["detail"] is not None:
                        for k,v in results["metrics"]["detail"].items():
                            msg += k + "="
                            msg += ",".join(["%.2f" % (v_*100) for v_ in v])
                        msg = msg[:-1]
                    else:
                        msg += "None"
                    msg += "} "
